...
export interface FooterMenuItemTextConfig extends FooterMenuItemConfig {
  content: string;
  type: "text";
}
export interface FooterMenuItemHeadingConfig extends FooterMenuItemConfig {
  content: string;
  // number between 1 and 5 indicating h1 - h5
  headingSize?: 1 | 2 | 3 | 4 | 5;
  type: "heading";
}
export interface FooterMenuItemImageConfig extends FooterMenuItemConfig {
  type: "image";
  content: string;
}
interface AltConfig {
  ...
  footerLayout: {
    ...
    columns: {
      // on a 24 unit grid, how much width should the column take?
      span?: number | string;
      // you can specify a menu from the saleor dashboard
      menuName?: string;
      // optional css classes. tailwind is supported
      className?: string;
      // or manually specify the menu items
      items?: (FooterMenuItemTextConfig | FooterMenuItemHeadingConfig | FooterMenuItemImageConfig)[];
    }[];
  };
}

const altConfig: AltConfig = {
  name: import.meta.env.RAKKAS_SITE_NAME,
  showSearch: true,
  ...
  //     config: {
  //       merchantCode: import.meta.env.RAKKAS_TCO_MERCHANT_CODE,
  //     },
  //     onPay: (
  //       {
  //         email,
  ...
  //           }
  //         });
  //         window.TwoCoInlineCart.events.subscribe("fulfillment:finalized", () => {
