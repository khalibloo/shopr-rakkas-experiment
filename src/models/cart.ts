...
import { CartLinesAddMutationVariables, CartLinesAddMutation } from "@/mutations/types/CartLinesAddMutation";
import {
  CartCreateMutation,
...
import { CartLinesUpdateMutation, CartLinesUpdateMutationVariables } from "@/mutations/types/CartLinesUpdateMutation";
import { CART_LINES_UPDATE_MUTATION } from "@/mutations/CartLinesUpdate";
import { CartLineDeleteMutationVariables, CartLineDeleteMutation } from "@/mutations/types/CartLineDeleteMutation";
...
import { CartCompleteMutation, CartCompleteMutationVariables } from "@/mutations/types/CartCompleteMutation";
...
import { CartVoucherAddMutation, CartVoucherAddMutationVariables } from "@/mutations/types/CartVoucherAddMutation";
...
import { cartBadgeWithTokenQuery, cartBadgeWithTokenQueryVariables } from "@/queries/types/cartBadgeWithTokenQuery";
import { productDetailQuery_product, productDetailQuery_product_variants } from "@/queries/types/productDetailQuery";
...
const CartModel: CartModelType = {
  ...
  effects: {
    *create({ payload }, { call, put, select }) {
      try {
        const authenticated = yield select((state: ConnectState) => state.auth.authenticated);
        ...
        if (!authenticated && guestCartToken) {
          ...
          const email = authenticated ? undefined : payload?.email || guestEmail;
          ...
          const response: { data: CartCreateMutation } = yield call(client.mutate, {
    ...
    *addItem({ payload }, { call, put, select, take }) {
      try {
        ...
        if (!authenticated && !localEmail) {
          ...
          let checkout = yield select((state: ConnectState) => state.cart.checkout);
          const variables: CartLinesAddMutationVariables = {
            checkoutId: checkout.id,
            checkoutLines: [{ variantId, quantity }],
          };
          const response: { data: CartLinesAddMutation } = yield call(client.mutate, {
          ...
          const savedCallback = yield select((state: ConnectState) => state.cart.onAddItem);
          savedCallback?.();
          yield put({
            type: "setOnAddItemCallback",
        ...
        // Google Ecommerce - track add to cart
    ...
    *updateItem({ payload }, { call, put, select, take }) {
      try {
        ...
        const response: { data: CartLinesUpdateMutation } = yield call(client.mutate, {
    ...
    *deleteItem({ payload }, { call, put, select, take }) {
      try {
        ...
        const response: { data: CartLineDeleteMutation } = yield call(client.mutate, {
        ...
        // Google Ecommerce - track remove from cart
        if (config.gtmEnabled && product && variant) {
          window.dataLayer.push({
            event: "remove_from_cart",
            ecommerce: {
    ...
    *setBillingAddress({ payload }, { call, put, select, take }) {
      try {
        ...
        const errors = response.data.checkoutBillingAddressUpdate?.checkoutErrors;
        if (errors && errors.length > 0) {
          throw new APIException(errors);
        }
        payload?.onCompleted?.(response.data);
    ...
    *setShippingAddress({ payload }, { call, put, select, take }) {
      try {
        ...
        const errors = response.data.checkoutShippingAddressUpdate?.checkoutErrors;
        if (errors && errors.length > 0) {
          throw new APIException(errors);
        }
        payload?.onCompleted?.(response.data);
    ...
    *setShippingMethod({ payload }, { call, put, select, take }) {
      try {
        ...
        const response: { data: CartShippingMethodUpdateMutation } = yield call(client.mutate, {
        ...
        const errors = response.data.checkoutShippingMethodUpdate?.checkoutErrors;
        if (errors && errors.length > 0) {
          throw new APIException(errors);
        }
        payload?.onCompleted?.(response.data);
    ...
    *addVoucher({ payload }, { call, put, select, take }) {
      try {
        ...
        const response: { data: CartVoucherAddMutation } = yield call(client.mutate, {
    ...
    *removeVoucher({ payload }, { call, put, select, take }) {
      try {
        ...
        const response: { data: CartVoucherRemoveMutation } = yield call(client.mutate, {
    ...
    *createPayment({ payload }, { call, put, select, take }) {
      try {
        ...
        const response: { data: CartPaymentCreateMutation } = yield call(client.mutate, {
