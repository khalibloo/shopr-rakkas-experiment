...
const setToken = (token) => {
  ...
  storage().setItem("exp", `${parseInt(exp, 10) * 1000}`);
};
const getCsrfToken = () => storage().getItem("csrfToken");
const isTokenExpired = () => {
  const exp = storage().getItem("exp");
  if (exp) {
...
export const client: ApolloClient<any> = new ApolloClient({
  link: ApolloLink.from([
    ...
    new TokenRefreshLink({
      isTokenValidOrUndefined: () => isTokenExpired() || typeof getCsrfToken() !== "string",
      ...
      handleResponse: (operation, accessTokenField) => (response) => {
        ...
        if (errors.length > 0) {
          const invalidCsrfToken = errors.find((e) => e.code === "JWT_INVALID_CSRF_TOKEN");
          if (invalidCsrfToken) {
